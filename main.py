...
def Riprider():
    is_login = session['is_login']
    username = session['username']
    return render_template('main.html', username=f'{username}', text='ОПИСАНИЕ$Кавалерийский боец. Наносит 33% сплэш-урона целям в зоне поражения.', image='riprider.png')
...
def register():
    ...
    return render_template('register.html', title='Регистрация', form=form, log=is_login)
